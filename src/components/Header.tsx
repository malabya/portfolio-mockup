import Logo from '@/assets/logo.svg';
import MainNavigation from './MainNavigation';

export default function Header() {
  return (
    <header className="site-header bg-white fixed top-0 start-0 w-full z-20 border-b border-gray-300">
      <div className="site-nav container px-2 py-4 mx-auto flex items-center justify-between">
        <div className="site-branding">
          <a href="/" className="site-branding__logo flex items-center gap-2">
            <Logo width={40} height={40} className="site-branding__logo" />
            <span className="font-metro font-bold text-4xl self-center whitespace-nowrap hidden md:block">Malabya</span>
          </a>
        </div>
        <div className='site-cta flex items-center gap-2 md:order-2'>
          <a href="#" className='px-4 py-2 bg-primary text-white rounded-2xl font-bold'>Get in Touch</a>
        </div>
        <div className='site-navigation items-center justify-between hidden w-full md:flex md:w-auto md:order-1' id="navbar-cta">
          <MainNavigation />
        </div>
      </div>
    </header>
  );
}
